<template>
    ...
    >
        ...
        <span
            :class="fillerClasses"
            :style="inputValue ? '' : `min-width: ${minWidth}px;`"
        >
            {{ fillerContent }}
        </span>
        <input
            ...
            :class="[
                ...
                'hover:border-gray-300 focus:border-primary-600 focus:outline-none',
                inline ? 'py-1' : 'py-2',
...
const minWidth = $computed(() => {
    if (!attrs.placeholder) return 60;

    return measureHTMLDimensions(
        `<span class="${fillerClasses}">${attrs.placeholder}</span>`,
    ).width;
});
