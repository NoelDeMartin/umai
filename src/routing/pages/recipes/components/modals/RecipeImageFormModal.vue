    ...
    <AppModal v-slot="{ close }" :title="$t('recipes.image_edit_title')">
        <BaseForm :form="form" class="flex flex-col" @submit="submit(close)">
            <div v-if="url">
                <RecipeImage
                    :url="url"
                    class="aspect-[5/2] max-h-[60vh] w-full min-w-modal-content"
                />
                <div
                    class="mt-2 flex items-center"
                    :title="
                        isUploadPending ? $t('recipes.image_edit_localUrl') : ''
                    "
                >
                    <i-zondicons-time
                    ...
                    <pre
                        v-if="!isLocalUrl"
                        class="align-center flex min-h-clickable items-center overflow-auto whitespace-pre-wrap text-gray-700"
                    >{{ url }}</pre>
                </div>
            </div>
            <template v-else>
                <div>
                    <div
                        class="mt-1 flex w-96 justify-center rounded-md border-2 border-dashed border-gray-300 px-6 pt-5 pb-6"
                    >
                        <div class="space-y-1 text-center">
                            ...
                            <div class="flex text-sm text-gray-600">
                                <BaseFileInput
                                    ...
                                    @start="
                                        $ui.showLoading(
                                            $t('recipes.image_edit_uploading')
                                        )
                                    "
                ...
                >{{ $t('recipes.image_edit_customUrl') }}:</span>
                <BaseInput
                    ref="input"
                    name="customUrl"
                    class="mt-2"
            ...
            <div
                class="mt-4 flex w-full flex-col space-y-2 md:flex-row md:space-x-2 md:space-y-0 md:self-end"
            >
                <BaseButton
                    v-if="url"
                    clear
...
import type {
    RecipeImageFormModalProps,
    RecipeImageFormModalResult,
} from './RecipeImageFormModal';
